...
export async function getAggregateReportPreviews(
  ...
  if (instanceIds.length === 0) {
    return {
      quantitative: { totalConversations: 0, totalMessages: 0, responseRate: 0, averageFirstResponseMinutes: 0, averageMessagesPerConversation: 0 },
      qualitative: { patterns: [], opportunities: [], objections: [] },
    };
  }
  if (instanceIds.length === 1) return getReportPreviews(instanceIds[0], from, to);

  const dateFilter = from ? { gte: from, lte: to } : undefined;
  const [conversations, messageCount] = await Promise.all([
    prisma.conversation.findMany({ where: { instanceId: { in: instanceIds }, ...(dateFilter ? { startedAt: dateFilter } : {}) } }),
    prisma.message.count({ where: { instanceId: { in: instanceIds }, ...(dateFilter ? { sentAt: dateFilter } : {}) } }),
  ]);
  ...
  const avgFirstResponseMinutes =
    withTmp.length > 0
      ? Math.round((withTmp.reduce((acc, c) => acc + (c.firstResponseTimeSecs ?? 0), 0) / withTmp.length / 60) * 10) / 10
      : 0;
  const avgMessagesPerConversation = total > 0 ? Math.round((messageCount / total) * 10) / 10 : 0;

  return {
    quantitative: { totalConversations: total, totalMessages: messageCount, responseRate, averageFirstResponseMinutes: avgFirstResponseMinutes, averageMessagesPerConversation: avgMessagesPerConversation },
    qualitative: { patterns: [], opportunities: [], objections: [] },
  };
}

export async function getQuantitativeReportAggregate(
  instanceIds: string[],
  from: Date | null,
  to: Date,
): Promise<QuantitativeReport> {
  if (instanceIds.length === 1) return getQuantitativeReport(instanceIds[0], from, to);
  ...
  const [conversations, inboundMessages] = await Promise.all([
    prisma.conversation.findMany({
      where: { ...where, ...(dateFilter ? { startedAt: dateFilter } : {}) },
      select: { inboundCount: true, outboundCount: true, messageCount: true, firstResponseTimeSecs: true, startedAt: true },
    }),
    prisma.message.findMany({
      where: { ...where, fromMe: false, ...(dateFilter ? { sentAt: dateFilter } : {}) },
      select: { sentAt: true },
    }),
  ]);
  ...
  const withTmp = conversations.filter((c) => c.firstResponseTimeSecs != null);
  const validTmp = withTmp.filter((c) => { const s = c.firstResponseTimeSecs ?? 0; return s >= 6 && s <= 86400; });
  ...
  const outlierTmp = withTmp.filter((c) => { const s = c.firstResponseTimeSecs ?? 0; return s < 6 || s > 86400; }).length;
  ...
  const volumeByDate: VolumeByDate[] = Array.from(dateMap.entries())
    ...
    .map(([isoDate, count]) => {
      ...
      return { date: `${day}/${month}/${year}`, dayOfWeek: dayNamesPt[d.getUTCDay()], count, isPeak: count === maxDateCount && maxDateCount > 0 };
    });
